...
<ul>
  ...
  <li><a href="/feide/logout?user=#{session[:feide_subject_id]}&session_index=#{session[:feide_session_index]}&relay_state=bar">Logout</a>
EOT
end
...
post meta.sp_assertion_consumer_service.location.path do
  ...
  session[:feide_subject_id] = r.assertions.first.subject.name_id
  session[:feide_session_index] = an.session_index
