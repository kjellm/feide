...
class FeideSP
  ...
  def call(env)
    ...
    puts "PATH #{request.request_method} #{request.path_info}"
    

   if request.request_method == 'GET' && request.path_info == '/feide/signon'
     puts "HERE"
     ...
     response.write(str)
     return response
   elsif request.request_method == 'GET' && request.path_info == '/feide/logout'
